...
public class Menu {
    ...
    /*Notas importantes de estudio:

JOptionPane: Se usa para mostrar diálogos gráficos de entrada y salida de datos,
ideal para programas de prueba sin interfaz web.
Map<String, Integer> operations: Permite asociar un nombre de operación con un índice numérico para usar switch.
Bucle while(true): Mantiene activo el menú hasta que el usuario selecciona "Exit".
Conversión String → int: Para operaciones sobre ID de usuario se convierte el input a int con Integer.parseInt().
Interacción con Repository: La clase no maneja SQL directamente; delega todas las operaciones al
repositorio (UserRepositoryImpl).
Control de nulos: Se valida que el usuario no cierre los diálogos antes de procesar los datos,
